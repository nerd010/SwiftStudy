...
struct Artist {
  ...
  static func artistsFromBundle() -> [Artist] {
    ...
    do {
      ...
      guard let roootObject = try JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [String: Any] else {
        return artists
      }
      
      guard let artistObjects = roootObject["artists"] as? [[String: AnyObject]] else {
        return artists
      }
